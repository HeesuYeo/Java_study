...
public class S17_MouseListener {
	...
	// MouseListener : 인터페이스, 모든 추상 메서드를 구현해야 해서 불편.
	...
	public static void main(String[] args) {
		...
		JButton bottom = new JButton("아래");
		
		//버튼이 아니더라도 모든 컴포넌트에는 사용 할 수 있는 공통 이벤트들이 있다
		bottom.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				...
				//마우스의 위치
				System.out.printf("마우스 좌표 (프로그램 에서) : (%d, %d)\n",
						e.getX(), e.getY());
				System.out.printf("마우스 좌표 (모니터상 에서) : (%d, %d)\n",
						e.getXOnScreen(), e.getYOnScreen());
				
				// 이벤트를 발생시킨 컴포넌트의 인스턴스 (컴포넌트 타입이므로 다운캐스팅 필요)
