/*
		# Java.util.Arrays
 		...
 		# Java.util.Collections
 			...
 			- 컬랙션을 편하게 다룰 수 있는 메서드들이 모여있는 클래스
...
public class D05_Collections {
	...
	public static void main(String[] args) {
		...
		// Collections.addAll(Collection, ...) : 해당 컬랙션에 여러 데이터를 한번에 추가하는 메서드
		...
		// Collections.copy(dest, src) : 불편함 (복사하는 컬랙션 크기가 더 작은 경우 에러)
		...
		// Collections.fill(list, item) : 해당 리스트의 모든 갓을 원하는값으로 초기화
		Collections.fill(copied, 7.777);
		System.out.println(copied);

		// Collections.frequency(list, item) : 리스트에 해당 값이 몇 개 들어있는지 알려주는
		List<String> fruits = new ArrayList<>();

		Collections.addAll(fruits, "apple", "apple", "banana", "apple", "kiwi", "peach");
		int a = Collections.frequency(fruits, "apple");
		System.out.printf("apple 는 %d번 등장.\n", a);

		// Collections.max(coll) : 해당 컬랙션에서 가장 큰 값을 하나 꺼내 반환
		// Collections.min(coll) : 해당 컬랙션에서 가장 작은 값을 하나 꺼내 반환
		...
		// Collections.rotate(list, distance) : 해당 리스트를 원하는 만큼 순환
		for (int i = 0; i <= 10; ++i) {
			Collections.rotate(cardList, 1);
			System.out.printf("rotate %d : %s\n", i, cardList);
		}
		// Collections.swap(list, i, j) : i 와 j의 위치의 값을 교체
		...
		// Collections.reverse(list) : 해당 리스트를 거꾸로 뒤집음

		System.out.println("뒤집기 전 : " + cardList);
		Collections.reverse(cardList);
		System.out.println("리버스 : " + cardList);

		// Collections.sort(list) : 해덩 리스트를 정렬
		...
		//카드가 들어있는 컬랙션은 아직 정렬 할 수 없다 (정렬 기준이 없기 때문에)
